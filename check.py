    ...
    def regression():
        ...
        with st.form('Regression'):
            ...
            with col3:
                ...
                st.write('')
                st.write('')
                button = st.form_submit_button(label='PREDICT SELLING PRICE BUTTON')
                style_submit_button()


        # give information to usersS
        ...
        if button:
            ...
            with open(r'/Users/karthickkumar/Desktop/copper/regression_model.pkl', 'rb') as f:
                model = pickle.load(f)
    ...
    def classification():
        ...
        with st.form('Classification'):
            ...
            with col3:
                ...
                st.write('')
                st.write('')
                button = st.form_submit_button(label='PREDICT STATUS BUTTON')
                style_submit_button()
        ...
        if button:
            ...
            with open(r'/Users/karthickkumar/Desktop/copper/classification_model.pkl', 'rb') as f:
                model = pickle.load(f)
...
st.sidebar.title("STEPS OF THE PROJECT")

selected_option = st.sidebar.selectbox("Select an Option", ["TECHNOLOGY USED","1. Data preprocessing", "2. Exploratory Data Analysis", "3. Classification","4. Regression"])
if selected_option=="TECHNOLOGY USED":
    st.sidebar.header("1. Python")
    st.sidebar.header("2. Pandas")
    st.sidebar.header("3. Numpy")
    st.sidebar.header("4. Matplotlib")
    st.sidebar.header("5. Seaborn")
    st.sidebar.header("6. Pickle")
    st.sidebar.header("7. Streamlit")   
elif selected_option=="1. Data preprocessing":
    st.sidebar.header("Data Understanding")
    st.sidebar.header("Handling Null Values")
    st.sidebar.header("Encoding and Data Type Conversion")
    st.sidebar.header("Skewness - Feature Scaling")
    st.sidebar.header("Outliers Handling")
    st.sidebar.header("Wrong Date Handling")
elif selected_option=="2. Exploratory Data Analysis":
    st.sidebar.header("Skewness Visualization")
    st.sidebar.header("Outlier Visualization")
    st.sidebar.header("Feature Improvement")
elif selected_option=="3. Classification":
    st.sidebar.header("Success and Failure Classification")
    st.sidebar.header("Handling Data Imbalance")
    st.sidebar.header("Algorithm Assessment")
    st.sidebar.header("Algorithm Selection")
    st.sidebar.header("Hyperparameter Tuning with GridSearchCV and Cross-Validation")
    st.sidebar.header("Model Accuracy and Metrics")
    st.sidebar.header("Model Persistence")

elif selected_option=="4. Regression":
    st.sidebar.header("Algorithm Assessment")
    st.sidebar.header("Algorithm Selection")
    st.sidebar.header("Hyperparameter Tuning with GridSearchCV and Cross-Validation")
    st.sidebar.header("Model Accuracy and Metrics")
...
with tab1:
    ...
    with col1:
        st.image("/Users/karthickkumar/Desktop/copper/image.png")
        st.image("/Users/karthickkumar/Desktop/copper/1626810204144.jpeg")

    # Content for the second column
    with col2:
        st.subheader("Machine learning with our Industrial Copper Modeling project is used to the copper industry, dealing with complex sales and pricing data can be challenging. Our solution employs advanced machine learning techniques to address these challenges, offering regression models for precise pricing predictions and lead classification for better customer targeting.")
with tab2:
    try:
